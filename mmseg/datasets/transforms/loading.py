...
@TRANSFORMS.register_module()
class LoadTifAnnotations(MMCV_LoadAnnotations):
    """Load annotations for semantic segmentation provided by dataset.
    ...
    def _load_seg_map(self, results: dict) -> None:
        ...
        gt_semantic_seg = tifffile.imread(results['seg_map_path'])
        results['gt_seg_map'] = gt_semantic_seg
        results['seg_fields'].append('gt_seg_map')
...
@TRANSFORMS.register_module()
class LoadTifAnnotationsV2(MMCV_LoadAnnotations):
    ...
    def _load_seg_map(self, results: dict) -> None:
        ...
        gt_semantic_seg = sio.imread(results['seg_map_path'])
        results['gt_seg_map'] = gt_semantic_seg
        results['seg_fields'].append('gt_seg_map')
...
@TRANSFORMS.register_module()
class LoadTifImageFromFileV2(BaseTransform):
    """Load a Remote Sensing mage from file.
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        filename = results['img_path']
        img = sio.imread(filename)      # (h, w, c), c=5, r-g-b-nir-mask
...
@TRANSFORMS.register_module()
class LoadTifImageFromFileV3(BaseTransform):
    """Load a Remote Sensing mage from file.
...
@TRANSFORMS.register_module()
class LoadTifImageFromFileV4(BaseTransform):
    """Load a Remote Sensing mage from file.
    ...
    def transform(self, results: Dict) -> Dict:
        ...
        filename = results['img_path']
        img = sio.imread(filename)      # (h, w)
        img = np.stack([img, img, img], axis=-1)          # r-r-r
