...
class EncoderDecoder(BaseSegmentor):
    ...
    def slide_inference(self, inputs: Tensor,
        ...
        for h_idx in range(h_grids):
            for w_idx in range(w_grids):
                ...
                # padding to the crop size if the crop_img is smaller than expected one.
                def pad_image(img, target_size):
                    """Pad an image up to the target size."""
                    h_missing = target_size[0] - img.shape[2]  # h
                    w_missing = target_size[1] - img.shape[3]  # w
                    if h_missing > 0 or w_missing > 0:
                        padded_img = F.pad(img, (0, w_missing, 0, h_missing), 'constant', 0)
                        return padded_img
                    else:
                        return img
                crop_img = pad_image(crop_img, (h_crop, w_crop))
                ...
                # cropping out the origin img
                crop_seg_logit = crop_seg_logit[:, :, :y2 - y1, :x2 - x1]
    ...
    def slide_inference_without_pad(self, inputs: Tensor, batch_img_metas: List[dict]) -> Tensor:
        """Inference by sliding-window with overlap.
