...
class OverlapPatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=7, stride=4, in_channels=3, embed_dim=768, norm_cfg=None):
        super().__init__()
...
class LSKNet(BaseModule):
    def __init__(self, img_size=224, in_channels=3, embed_dims=(64, 128, 256, 512),
...
if __name__ == '__main__':
    lsk = LSKNet(img_size=512, in_channels=3, embed_dims=[64, 128, 256, 512],
                 mlp_ratios=[8, 8, 4, 4], drop_rate=0., drop_path_rate=0., norm_layer=partial(nn.LayerNorm, eps=1e-6),
                 depths=[3, 4, 6, 3], num_stages=4,
                 pretrained=None,
                 init_cfg=None,
                 norm_cfg=None)
    img_ = torch.rand((2, 3, 512, 512)).float()
    out_ = lsk(img_)
    for o_ in out_:
        print(o_.shape)
